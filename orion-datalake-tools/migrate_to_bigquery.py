...
def migrate_with_pandas():
    ...
    データ型を強制的に変換し、クリーンなDataFrameをBigQueryにロードする。
    """
    # 環境変数から設定を読み込み
    ...
    try:
        df = pd.read_csv(gcs_uri, on_bad_lines='skip')
        print(f"読み込み成功。{len(df)}行の有効なデータを検出しました。")

        # データ型を強制的に変換
        print("データ型をBigQueryスキーマに合わせて強制的に変換します...")
        ...
        print(f"PandasでのCSV読み込みまたは型変換中にエラーが発生しました: {e}")
        return
